...
export default function Login() {
  ...
  // If the person just landed on /login directly (no explicit redirect from
  // another page), check whether they're an admin and send them straight
  // to the dashboard instead of the regular account page.
  const resolveRedirect = async (userId) => {
    if (location.state?.redirect) return redirectTo;
    try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === "signin") {
        ...
        navigate(await resolveRedirect(user.id), { replace: true });
      } else {
