...
export default function ProductDetail() {
  ...
  useSEO({
    title: product
      ? `${product.name} | Royal Wedding Cards`
      : "Wedding Card Design | Royal Wedding Cards",
    description: product
      ? `${product.name} — ${product.description || "premium customizable invitation design"}. View price and order from Royal Wedding Cards.`
      : "View wedding invitation card design details.",
    path: `/product/${id}`,
    type: "product",
    image: product?.image,
    structuredData: product
      ? {
          ...
          brand: { "@type": "Brand", name: "Royal Wedding Cards" },
          offers: {
            "@type": "Offer",
            priceCurrency: "INR",
            price: product.price,
  ...
  const handleWhatsappOrderClick = () => {
    ...
    if (!profile?.full_name || !profile?.phone || !profile?.address) {
      navigate("/account", {
        state: { needsProfile: true, redirect: `/product/${id}` },
      });
      return;
    }
  ...
  const handleConfirmWhatsappOrder = async () => {
    ...
    try {
      await createWhatsappOrder(product, 1, user?.id ?? null, {
        customerName: profile.full_name,
        customerPhone: profile.phone,
        customerAddress: profile.address,
      });
      const waMessage = buildWhatsappOrderMessage(product, 1, {
        account: {
        ...
        <div className="flex flex-col justify-between">
          <div>
            <div className="flex items-start justify-between gap-4 mb-3">
              <h1 className="text-3xl md:text-4xl font-heading font-bold text-[#2B2118] tracking-tight">
                {product.name}
              </h1>
              <button
            ...
            <div className="flex items-center gap-2 text-[#D4AF37] mb-4 font-medium text-sm">
              <div className="flex items-center gap-1">
                ...
                <span className="text-[#2B2118] font-bold">
                  {product.rating}
                </span>
              </div>
              <span className="text-neutral-300">•</span>
              <span className="text-neutral-500">
                ({product.reviews?.length ?? product.ratingCount ?? 0} review
                {(product.reviews?.length ?? product.ratingCount ?? 0) === 1
                  ? ""
                  : "s"}
                )
              </span>
            ...
            <p className="text-3xl font-bold text-[#8B1E3F] mb-6">
              ₹{product.price}
            </p>

            <p className="text-neutral-600 mb-6 leading-relaxed text-sm md:text-base">
              A beautifully crafted invitation designed to set the tone for your
              celebration. Fully customizable with your names, dates, and
              wording — printed on premium card stock with meticulous royal
              finishing.
            </p>
              ...
              <div className="flex items-center border border-black/10 rounded-2xl bg-[#fffaf5] shadow-sm">
                <button
                  onClick={() => {
                    ...
                    if (!current) return;
                    if (current.qty <= 1) removeFromCart(product.id);
                    else updateQty(product.id, current.qty - 1);
                  }}
                  className="p-3.5 text-neutral-700 hover:text-[#8B1E3F]"
                >
                  <Minus size={16} />
                </button>
                <span className="w-10 text-center font-bold text-[#2B2118]">
                  {items.find((i) => i.id === product.id)?.qty ?? 0}
                </span>
                <button
                  onClick={() => {
                    const current = items.find((i) => i.id === product.id);
                    if (current) updateQty(product.id, current.qty + 1);
      ...
      <section className="mt-28 grid lg:grid-cols-[1fr_400px] gap-12 items-start">
        <div>
          <div className="flex items-baseline gap-3 mb-8 border-b border-black/5 pb-4">
            <h2 className="text-2xl font-heading font-bold text-[#2B2118]">
              Customer Reviews
            </h2>
            <span className="text-neutral-500 text-sm font-medium">
              {product.reviews?.length || 0} total · {product.rating} average
              rating
            </span>
          </div>
          ...
          {product.reviews?.length ? (
            <div className="space-y-6">
              {product.reviews.map((r) => (
                <div
                  key={r.id}
                  className="bg-[#fffaf5] border border-black/5 rounded-2xl p-6 shadow-sm"
                >
                  <div className="flex items-center justify-between mb-2">
                    <p className="font-heading font-semibold text-[#2B2118]">
        ...
        <div className="bg-[#fffaf5] border border-black/10 rounded-3xl p-8 shadow-sm h-fit sticky top-28">
          <h3 className="font-heading font-bold text-xl mb-2 text-[#2B2118]">
            Write a Review
          </h3>
          <p className="text-xs text-neutral-500 mb-6">
            Share your thoughts and feedback about this handcrafted design.
          </p>
        ...
        createPortal(
          <div className="fixed inset-0 z-[90] flex items-end sm:items-center justify-center bg-black/50 p-4">
            <div className="w-full sm:max-w-md bg-white dark:bg-neutral-900 rounded-3xl p-6 shadow-2xl">
              <h2 className="font-heading font-bold text-xl mb-1 text-[#2B2118] dark:text-white">
                Confirm Your Order
              </h2>
              <p className="text-sm text-neutral-500 mb-5">
                Please check the details below before placing your order.
              </p>

              <div className="flex items-center justify-between text-sm mb-3">
                <span className="truncate pr-2">{product.name} × 1</span>
              ...
              <div className="border-t border-black/5 dark:border-white/10 pt-3 mb-4 space-y-1">
                <p className="text-xs text-neutral-500">
                  Contact: {profile?.full_name} · {profile?.phone}
                </p>
                <p className="text-xs text-neutral-500">
                  📍 {profile?.address}
                </p>
